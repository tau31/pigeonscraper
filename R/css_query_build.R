...
#'
#' @param years tibble or data.drame with year information and year css
#' queries generated by \code{\link{extract_years}}.
#' @param remDr Object class remote driver previosly connected using
#' \code{\link{connect_remDr}}.
#' @return \code{tibble} with years, organization name and css queries for
#' both years and organizations.
#'
#' @import RSelenium
    ...
    orgs <- purrr::map_dfr(
      ...
      function(y) {
        ...
        temp_orgs <-
          ...
          dplyr::filter(
            stringr::str_detect(.data$organization, "Click", negate = TRUE)
            ) %>%
